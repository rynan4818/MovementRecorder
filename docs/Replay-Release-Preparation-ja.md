...
2026-09-13。現在のブランチは **BS1.37.4 / MovementRecorder 0.3.2 / Beat Saber 1.37.4-1.39.1** です。採番の変更を反映した[設計第3版](Replay-Release-Branches-Plan-v3-ja.md)を基準にしています。
...
スクリプトは同じcsprojをVisual StudioのMSBuildで呼び出します。このブランチの基準となるゲームは1.37.4です。

```powershell
./scripts/Build-Replay.ps1 -GameDirectory 'C:/Program Files (x86)/Steam/steamapps/common/Beat Saber_1.37.4'
./scripts/Test-ReleasePackage.ps1
./scripts/Test-ReplayContracts.ps1 -GameDirectory 'C:/Program Files (x86)/Steam/steamapps/common/Beat Saber_1.37.4'
./scripts/Test-AssemblyReferences.ps1 -GameDirectory 'C:/Program Files (x86)/Steam/steamapps/common/Beat Saber_1.37.4'
...
ZIPはBeatSaberModdingTools.Tasks 2.0.0-beta1がMovementRecorder/bin/Release/zipへ自動生成します。このブランチの名前は `MovementRecorder-0.3.2-bs1.37.4-<commit>.zip` です。対応範囲の終端は付けず、名前変更や独自のZIP作成は行いません。ビルド時のゲームへのコピーは無効です。
